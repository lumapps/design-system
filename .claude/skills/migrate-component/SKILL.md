...
**IMPORTANT RULES:**
- **NO JSX ELEMENTS for custom components in core stories** - Use plain data or functional JSX calls, NOT JSX elements like `<Component />`
...
3. **NO JSX ELEMENTS or component calls in core stories or tests** - Use plain data only, provide components via `overrides` (stories) or framework-specific tests (tests)
...
6. **Check component dependencies before migrating stories** - If a story uses components not in core, don't migrate it
...
12. **Vue stories with component children need `.vue` templates in `overrides`** - Create `.vue` template and use `render: withRender({ ComponentVue })` in overrides
13. **Vue components with `children` need base `.vue` template + `withRender`** - Convert children prop to slots
14. **Don't override Vue combination decorators with JSX** - Can't use JSX in `withCombinations` rows; use core version or create templates
15. **Don't pass JSX components in Vue `args.children`** - Use `render: withRender({ ComponentVue })` instead
16. **Use `.ts` extension for Vue stories** - Only use `.tsx` if absolutely necessary (rare)
17. **Vue tests must mimic React tests** - Include the same structure with `commonTestsSuiteVTL`
18. **Vue `index.ts` should match React `index.ts` structure** - Export the same items (components, types, constants) in the same order, only differing in default vs named export syntax for components
19. **Always check for existing core implementation first** - Before creating UI/Stories/Tests in core, verify they don't already exist. If they do, reuse them and only make changes after user approval
20. **Use ReactToJSX and VueToJSXProps type utilities** - Don't manually list all props to omit; use the new type utilities that automatically handle PropsToOverride
