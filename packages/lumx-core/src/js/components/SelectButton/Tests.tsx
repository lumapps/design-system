...
export function createTemplates(SelectButton: any) {
    ...
    return { defaultTemplate, multiTemplate };
}
...
export default function selectButtonTests({ components, renderWithState }: SelectButtonTestSetup) {
    ...
    const { defaultTemplate, multiTemplate } = createTemplates(SelectButton);
    ...
    describe('Multiple selection', () => {
        ...
        it('should mark all selected options with aria-selected', async () => {
            ...
            await waitFor(() => {
                ...
                expect(options[1].getAttribute('aria-selected')).toBe('false'); // Apricot
                expect(options[2].getAttribute('aria-selected')).toBe('true'); // Banana
                expect(options[3].getAttribute('aria-selected')).toBe('false'); // Blueberry
            });
        });

        it('should add an option and keep the dropdown open after selecting', async () => {
            renderWithState(multiTemplate, { value: [] });
            const button = screen.getByRole('combobox');
            ...
            await waitFor(() => {
                // Dropdown stays open thanks to aria-multiselectable.
                expect(button.getAttribute('aria-expanded')).toBe('true');
                // Apple is now selected.
                const optionsNow = screen.getAllByRole('option').filter((o) => !o.hasAttribute('data-filtered'));
                expect(optionsNow[0].getAttribute('aria-selected')).toBe('true');
                // Trigger displays the selected option name.
                expect(button.textContent).toContain('Apple');
            });
        });

        it('should toggle off an option when clicking it again', async () => {
            renderWithState(multiTemplate, { value: [FRUITS[0], FRUITS[2]] }); // Apple, Banana
            const button = screen.getByRole('combobox');
            ...
            await waitFor(() => {
                const optionsNow = screen.getAllByRole('option').filter((o) => !o.hasAttribute('data-filtered'));
                expect(optionsNow[0].getAttribute('aria-selected')).toBe('false');
                expect(optionsNow[2].getAttribute('aria-selected')).toBe('true'); // Banana still selected
            });
        });

        it('should display pre-selected option names joined with ", " in the trigger', () => {
            renderWithState(multiTemplate, { value: [FRUITS[0], FRUITS[2], FRUITS[7]] }); // Apple, Banana, Orange
            const button = screen.getByRole('combobox');
            expect(button.textContent).toContain('Apple, Banana, Orange');
        });

        it('should display the label when no values are selected', () => {
            renderWithState(multiTemplate, { value: [] });
            const button = screen.getByRole('combobox');
            expect(button.textContent).toContain('Select fruits');
        });
    });
}
