...
describe(snapTimeToBounds, () => {
    it('returns the input unchanged when no bounds are provided', () => {
        ...
        expect(snapTimeToBounds(time, 30)).toBe(time);
    });

    it('returns the input unchanged when within bounds', () => {
        const time = { hour: 14, minute: 30 };
        expect(
            snapTimeToBounds(time, 30, getDateAtTime({ hour: 8, minute: 0 }), getDateAtTime({ hour: 18, minute: 0 })),
        ).toBe(time);
    });

    it('snaps up to `minTime` when input is below it', () => {
        const time = { hour: 6, minute: 0 };
        expect(snapTimeToBounds(time, 30, getDateAtTime({ hour: 8, minute: 30 }))).toEqual({ hour: 8, minute: 30 });
    });

    it('snaps down to `maxTime` when input is above it', () => {
        const time = { hour: 22, minute: 0 };
        expect(snapTimeToBounds(time, 30, undefined, getDateAtTime({ hour: 18, minute: 0 }))).toEqual({
            hour: 18,
            minute: 0,
        });
    });
    ...
    it('ignores the date part of the bounds', () => {
        ...
        expect(snapTimeToBounds(time, 30, oldDate)).toEqual({ hour: 9, minute: 0 });
    });

    it('snaps up to the next on-grid option when `minTime` is not step-aligned', () => {
        const time = { hour: 9, minute: 0 };
        expect(snapTimeToBounds(time, 30, getDateAtTime({ hour: 10, minute: 5 }))).toEqual({
            hour: 10,
            minute: 30,
        });
    });
    ...
    it('snaps down to the last on-grid option when `maxTime` is not step-aligned', () => {
        ...
        expect(snapTimeToBounds(time, 30, undefined, getDateAtTime({ hour: 10, minute: 5 }))).toEqual({
            hour: 10,
            minute: 0,
        });
    });
    ...
    it('does not change behavior when the bound already sits on the grid', () => {
        ...
        expect(snapTimeToBounds(time, 30, getDateAtTime({ hour: 8, minute: 0 }))).toEqual({ hour: 8, minute: 0 });
    });

    it('keeps the value unchanged when no on-grid option exists at/after `minTime`', () => {
        const time = { hour: 23, minute: 50 };
        expect(snapTimeToBounds(time, 30, getDateAtTime({ hour: 23, minute: 45 }))).toBe(time);
    });

    it('keeps the value unchanged when `minTime` and `maxTime` leave no on-grid option between them', () => {
        const time = { hour: 10, minute: 10 };
        expect(
            snapTimeToBounds(time, 30, getDateAtTime({ hour: 10, minute: 5 }), getDateAtTime({ hour: 10, minute: 20 })),
        ).toBe(time);
    });
});
