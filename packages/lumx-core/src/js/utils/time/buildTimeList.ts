...
export interface TimeOfDay {
    ...
    /** 0–59 */
    minute: number;
    /** Stable string id (`hour * 60 + minute`). Populated by `buildTimeList`. */
    id?: string;
...
export function buildTimeList(options: BuildTimeListOptions = {}): TimeOfDay[] {
    ...
    for (let minutes = 0; minutes < MINUTES_IN_DAY; minutes += step) {
        ...
        const outOfRange =
            ...
            undefined;
        const name = locale ? formatTime(getTime(hour, minute), locale) : undefined;

        list.push({ hour, minute, id, name, outOfRange });
    }
    return list;
}
