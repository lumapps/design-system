...
const setup = (props: Partial<ImageCaptionProps> = {}) => {
    const { container } = render(<ImageCaption baseClassName={BASE_CLASS} {...(props as any)} />);
    const wrapper = queryByClassName(container, `${BASE_CLASS}__wrapper`);
...
describe('<ImageCaption>', () => {
    ...
    it('should render when tags are provided', () => {
        const { wrapper } = setup({ tags: <span>Tag 1</span> });
        expect(wrapper).toBeInTheDocument();
    ...
    describe('caption elements', () => {
        ...
        it('should render description from HTML object', () => {
            const { wrapper } = setup({ description: { __html: '<em>HTML Description</em>' } });
            const caption = queryByClassName(wrapper as HTMLElement, `${BASE_CLASS}__caption`);
            const description = queryByClassName(caption as HTMLElement, `${BASE_CLASS}__description`);
            expect(description).toBeInTheDocument();
            expect(description?.querySelector('em')).toHaveTextContent('HTML Description');
        });

        it('should render tags element with correct class', () => {
            const { wrapper } = setup({ tags: <span>Tag</span> });
    ...
    describe('Props', () => {
        ...
        it('should render as div when as="div"', () => {
            const { wrapper } = setup({ title: 'My Title', as: 'div' as any });
