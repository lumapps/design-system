...
describe(`<ButtonGroup>`, () => {
    const renderComponent = (props: ButtonGroupProps & { children?: any }, options: SetupRenderOptions<ButtonGroupProps> = {}) => {
    ...
    const setup = (propsOverride: Partial<ButtonGroupProps> = {}, options: SetupRenderOptions<ButtonGroupProps> = {}) => {
        const props = {
            children: [
                h(Button, () => 'Label'),
                h(Button, { rightIcon: mdiPlus, 'aria-label': 'Add' }),
            ],
            ...propsOverride,
        };
