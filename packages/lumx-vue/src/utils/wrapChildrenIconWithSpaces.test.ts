...
describe('wrapChildrenIconWithSpaces', () => {
    ...
    it('should not add a space when the adjacent text already has one', () => {
        const iconNode = h(Icon, { icon: 'some-icon' });

        const result = wrapChildrenIconWithSpaces(['text\u00a0', iconNode, '\u00a0more text']);

        expect(result).toHaveLength(3);
        expect(result![0]).toBe('text\u00a0');
        expect(result![1]).toBe(iconNode);
        expect(result![2]).toBe('\u00a0more text');
    });

    it('should not add a space when the adjacent text VNode already has one', () => {
        const iconNode = h(Icon, { icon: 'some-icon' });
        const before = createTextVNode('text\u00a0');
        const after = createTextVNode('\u00a0more text');

        const result = wrapChildrenIconWithSpaces([before, iconNode, after]);
