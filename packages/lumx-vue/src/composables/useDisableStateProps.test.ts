...
describe('useDisableStateProps', () => {
    ...
    const ChildComponent = defineComponent({
        ...
        setup(props) {
            const { disabledStateProps, otherProps } = useDisableStateProps(props.componentProps);
            return () => h('button', { 'data-testid': testId, ...otherProps.value, ...disabledStateProps.value });
        },
    });
    ...
    it('should forward onClick when not disabled', async () => {
        const onClick = vi.fn();
    ...
    it('should not forward onClick when disabled', async () => {
        const onClick = vi.fn();
