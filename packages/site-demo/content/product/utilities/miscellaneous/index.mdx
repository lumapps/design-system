...
<Message kind="info">
    In this example, clicking outside the card (referenced by <code>cardRef</code>) will trigger the <code>close</code>
    callback. Clicks in children DOM nodes of the card or children of nested "click away" contexts (here the dropdown) are
    all considered as "in" the the context and won't trigger the callback.
</Message>
