...
Mark each required field with `isRequired`: the label shows a marker (`*`) and the input gets the `required` attribute.
A form that contains required fields must explain the marker: place an
[input required legend](/product/components/input-required-legend/) before the first field, with a wording translated
