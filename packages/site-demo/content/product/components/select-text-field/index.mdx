...
Use `isRequired` to tell that the field must be filled to submit the form. The label shows a marker (`*`) and the input
gets `aria-required`. A form that contains required fields must explain the marker: place an
[input required legend](/product/components/input-required-legend/) before the first field.

Unlike `TextField`, the input does not get the native `required` attribute. The input text is not the value of the field:
in multiple selection the value is held by the chips and the input stays empty, and with `filter="off"` the input is read
only. Native form validation would therefore report a valid field as empty. Validate the selection yourself on submit.
