...
import { Autocomplete, List, ListItem } from '@lumx/react';
...
import { AutocompleteMultiple, Chip, ChipGroup, Icon, List, ListItem } from '@lumx/react';
...
export default ({ theme }) => {
    ...
    const selectedChipRender = (city, index) => (
        <Chip
            theme={theme}
            isClickable
            key={index}
