...
async function main({ inputs, github, context }) {
    ...
    const pull_number = inputs.pull_number ? parseInt(inputs.pull_number) : context.issue.number;
    ...
    if (!lastMatch) {
        ...
        updatedBody = body.replaceAll(pattern, (...groups) => {
            ...
            if (replaceText !== replace) console.log(`> Replace pattern resolved as: "${replaceText}"`);
            return replaceText;
        });
    }
