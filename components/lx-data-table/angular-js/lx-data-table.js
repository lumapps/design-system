(function IIFE() {
  ...
  function lxDataTableController($rootScope, $sce, $scope) {
    ...
    function sort(column) {
      ...
      for (var i = 0, len = lxDataTable.thead.length; i < len; i++) {
        if (
          lxDataTable.thead[i].sortable &&
          lxDataTable.thead[i].name !== column.name
        ) {
    ...
    function toggleSelection(row, newSelectedStatus, evt) {
      ...
      row.lxDataTableSelected = angular.isDefined(newSelectedStatus)
        ? newSelectedStatus
        : !row.lxDataTableSelected;

      if (row.lxDataTableSelected) {
        // Make sure it's not already in.
        if (
          lxDataTable.selectedRows.length === 0 ||
          (lxDataTable.selectedRows.length &&
            lxDataTable.selectedRows.indexOf(row) === -1)
        ) {
        ...
        if (
          ...
          lxDataTable.selectedRows.splice(
            lxDataTable.selectedRows.indexOf(row),
            1
          );
          lxDataTable.allRowsSelected = false;
  ...
  function lxDataTableDirective() {
    return {
      ...
      templateUrl: "components/lx-data-table/angular-js/lx-data-table.html"
    };
  }
  ...
  angular
    ...
    .directive("lxDataTable", lxDataTableDirective);
})();
