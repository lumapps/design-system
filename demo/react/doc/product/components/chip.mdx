...
(theme) => {
    ...
    return <>
        <Chip
            ...
            after={isSelected ? <Icon icon={mdiCloseCircle} size={Size.xs}/> : <Icon icon={mdiMenuDown} size={Size.xs} />}
...
(theme) => <>
    ...
    <Chip theme={theme} size={Size.s} before={<Icon icon={mdiClose} size={Size.xxs} />}>
        Dismissible
    </Chip>
</>
```
