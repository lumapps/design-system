...
(theme) => {
    ...
    const filteredCities = CITIES.filter((city) => {
        ...
        const wasCityAlreadySelected = selectedValues.filter(c => c.id === city.id).length > 0;

        return !wasCityAlreadySelected && noSpacesCity.includes(filterValue.replace(' ', '').toLowerCase());
    });
    const closeAutocomplete = () => setShowSuggestions(false);
